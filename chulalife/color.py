...
if __name__ == "__main__":
    ...
    # show example of using the colors
    ...
    while running:
        ...
        screen.blit(title_font.render("Color Sample", True, BLACK), (20, 20))
        for r, top in enumerate(range(80, HEIGHT, block_size + 10)):
            for c, left in enumerate(range(0, WIDTH, block_size + 10)):
                ...
                print(f"{top=} {r=} {left=} {c=}")
                pygame.draw.rect(
                    screen, color, (left, top, block_size, block_size))
                font = pygame.font.Font("assets/fonts/2005_iannnnnAMD.ttf", 22)
                text = font.render(str(color), True, BLACK)
                screen.blit(text, (left + 5, top + block_size + 5))
